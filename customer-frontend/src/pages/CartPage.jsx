...
import { menuAPI } from '../services/api'; // ADD THIS
import QuantityControl from '../components/QuantityControl';
import ConfirmDialog from '../components/ConfirmDialog';

const CartPage = () => {
  const { items, cartTotal, removeFromCart, updateQuantity, clearCart } =
    useCart();
  ...
  const validateCheckout = async () => {
    try {
      ...
      for (const item of items) {
        try {
          const response = await menuAPI.getItemById(item._id);
      ...
      if (unavailableItems.length > 0) {
        toast.error(
          `Some items are no longer available: ${unavailableItems.join(", ")}. Please remove them from cart.`,
        );
        return false;
      }
              ...
              >
                ...
                <div className="flex flex-col items-end space-y-3">
                  <QuantityControl
                    ...
                    onIncrease={() =>
                      handleQuantityIncrease(item.cartItemId, item.quantity)
                    }
                    onDecrease={() =>
                      handleQuantityDecrease(item.cartItemId, item.quantity)
                    }
                  />
