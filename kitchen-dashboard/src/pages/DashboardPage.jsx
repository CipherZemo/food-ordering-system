...
const DashboardPage = () => {
  ...
  const handleUndo = async () => {
    ...
    try {
      await kitchenAPI.updateOrderStatus(undoAction.orderId, undoAction.oldStatus);
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      {undoAction && (
        <div className="fixed top-20 right-4 z-50 bg-gray-800 text-white rounded-lg shadow-lg p-4 flex items-center space-x-3 animate-slide-in">
          ...
          <div className="flex-grow">
            <p className="font-medium">Order #{undoAction.orderNumber} moved to {undoAction.newStatus}</p>
          </div>
          <button
            onClick={handleUndo}
      ...
      {searchQuery && (
        <div className="container mx-auto px-4 pt-4">
          <p className="text-gray-600">
            Found {filteredOrders.length} {filteredOrders.length === 1 ? 'order' : 'orders'} for "{searchQuery}"
          </p>
        </div>
      )}
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">Ready for Pickup</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to logout? You'll need to login again to access the kitchen dashboard.
            </p>
            <div className="flex items-center space-x-3">
              <button
